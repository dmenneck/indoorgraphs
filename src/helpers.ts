...
/*
 * Input:
 * Output: return graph with new position
 * 
*/
/*
const matchToNearestPath = (graph: any, coords: number[], type: string, match: any) => {
  ...
  // 1. get closest node, 2. get closest node from coords to adjacend nodes of closest node, 3. calculate 
  // position on the line between these nodes
  if (!roomEntrancePresent) {
    Object.entries(graph.nodes).map(([id, attributes]: any) => {
      ...
      if (matchLevel === attributes.level) points.push(turf.point(coordinates, { id }))
      else if (!matchLevel) points.push(turf.point(coordinates, { id }))
    })

    if (points.length === 0) return { error: true, message: `No nodes found on floor ${matchLevel}.` }

    console.log(points.length)

    const pointsFeatureCollection = turf.featureCollection(points);
    const nearestNode = turf.nearestPoint(targetPoint, pointsFeatureCollection);
    ...
    adjacentNodes.map((n: string) => {
      ...
      const nodeId = n.includes(":") ? n.split(":")[0] : n;

      const node = graph.nodes[nodeId]

      if (!node) return;

      const coordinates = node[0];
      coordinates && nextPoints.push(turf.point(coordinates, { id: nodeId }))
      }
    })

    const nextPointsFeatureCollection = turf.featureCollection(nextPoints);
    const nextNearestNode = turf.nearestPoint(targetPoint, nextPointsFeatureCollection);
    const nextNearestNodeId = nextNearestNode.properties.id;
    ...
    const line = turf.lineString([
      ...
      nextNearestNode.geometry.coordinates
    ]);
    ...
    graph.nodes[`matchedTemp${type}`] = [
      ...
      '?',
      '',
      [nearestNodeId, nextNearestNodeId]
    ]
  }
